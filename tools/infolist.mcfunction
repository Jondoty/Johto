#Current Map Version
tellraw @s {"text":"1) Map version: Johto 3.2.4"}

scoreboard players tag @s add infoverify

#Spawn core commands load test
#execute @e[x=-880,y=64,z=-336,dy=3,type=armor_stand] ~ ~ ~ tellraw @a[tag=infoverify] {"text":"2a) Northwest spawn corner confirmed loaded","italic":true,"color":"green"}
#execute @e[x=-689,y=64,z=-336,dy=3,type=armor_stand] ~ ~ ~ tellraw @a[tag=infoverify] {"text":"2b) Northeast spawn corner confirmed loaded","italic":true,"color":"green"}
#execute @e[x=-689,y=95,z=-145,dy=3,type=armor_stand] ~ ~ ~ tellraw @a[tag=infoverify] {"text":"2c) Southeast spawn corner confirmed loaded","italic":true,"color":"green"}
#execute @e[x=-880,y=80,z=-145,dy=3,type=armor_stand] ~ ~ ~ tellraw @a[tag=infoverify] {"text":"2d) Southwest spawn corner confirmed loaded","italic":true,"color":"green"}



#tellraw @a[tag=infoverify] ["",{"text":"All spawn chunk corners confirmed loaded. ["},{"text":"Info","hoverEvent":{"action":"show_text","value":"These 4 corners must be loaded for the map to fully function. These are spawn chunks the world keeps loaded at all times, and runs most functions of the map."}},{"text":"]"}]





execute @e[x=-880,y=64,z=-336,dy=3,type=armor_stand] ~ ~ ~ execute @e[x=-689,y=64,z=-336,dy=3,type=armor_stand] ~ ~ ~ execute @e[x=-689,y=95,z=-145,dy=3,type=armor_stand] ~ ~ ~ execute @e[x=-880,y=80,z=-145,dy=3,type=armor_stand] ~ ~ ~ scoreboard players tag @a[tag=infoverify] add skip
...
execute @s[tag=!skip] ~ ~ ~ /setworldspawn -780 64 -245
...
execute @e[x=-753,y=65,z=-243,r=2,type=pixelmon:statue] ~ ~ ~ scoreboard players tag @a[tag=infoverify] add skip
